...
Namespace IbUkeharai.InquiryExec
	...
	Public Class BaseOperation2
		...
		Public Shared Function CopyToFile(src As String, dst As String) As Boolean
			If Not BaseOperation2.IsFile(src) Then
				Return False
			End If
			Try
				If BaseOperation2.IsFile(dst) Then
					File.Delete(dst)
				End If
				File.Copy(src, dst)
			Catch ex As Exception
		...
		Public Shared Function DeleteFile(path As String) As Object
			If Not BaseOperation2.IsFile(path) Then
				Return False
			End If
			Try
				File.Delete(path)
		...
		Public Shared Sub DeleteFileList(files As List(Of String), Optional num As Integer = 0)
			...
			For i As Integer = num2 To num3
				BaseOperation2.DeleteFile(files(i))
			Next
		End Sub
		...
		Public Shared Function GetFileList(path As String) As String()
			...
			If Not BaseOperation2.IsDirectory(path) Then
				Return result
			End If
			Try
				Dim filesOrderByDate As String() = BaseOperation2.GetFilesOrderByDate(path)
		...
		Public Shared Function GetFilesOrderByDate(path As String) As String()
			...
			Next
			Array.Sort(Of DateTime, FileInfo)(array1, files)
