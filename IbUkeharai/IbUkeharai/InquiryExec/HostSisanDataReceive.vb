	...
	Public Class HostSisanDataReceive
		...
		Private Function CheckConfigParam() As Boolean
			...
			If Operators.CompareString(xmlConfData.Kyoten, String.Empty, False) = 0 Then
				Return False
			End If
			Me._kyoten = CType(Me.GetKyotenNo(xmlConfData.Kyoten), HostSisanDataReceive.baseNum)
		...
		Private Function CheckSettingParam() As Boolean
			Return Not (Me._kyoten <> HostSisanDataReceive.baseNum.Matuyama And Me._kyoten <> HostSisanDataReceive.baseNum.Kumamoto) AndAlso Not (Information.IsNothing(Me._ftpSisanFile) Or Information.IsNothing(Me._ftpSisanFile)) AndAlso Not Information.IsNothing(Me._ftpSaveFolder) AndAlso Not Information.IsNothing(Me._ftpSaveFile) AndAlso Me.SetFtpTestMode(Me._ftpkeppin.TestMode)
		End Function
		...
		Private Function KeppinDataReceive() As Boolean
			Dim ftpsisanDataUrl As String = Me.GetFTPSisanDataUrl()
			Dim savePath As String = Me.GetSavePath()
			Dim ftpCommon As New FtpCommon(Me._ftpkeppin.UserId, Me._ftpkeppin.Password)
			If ftpCommon.IsDirectory(ftpsisanDataUrl) Then
				If Not ftpCommon.DownLoadFile(ftpsisanDataUrl, savePath) Then
		...
		Private Function GetFTPSisanDataUrl() As String
			Dim result As String = ""
			If Me._testmode Then
				Return result
			End If
			If Me._kyoten = HostSisanDataReceive.baseNum.Matuyama Then
				result = Path.Combine(Me._ftpkeppin.FtpUrl, Me._ftpSisanFile)
			ElseIf Me._kyoten = HostSisanDataReceive.baseNum.Kumamoto Then
				result = Path.Combine(Me._ftpkeppin.FtpUrl, Me._ftpSisanFile)
			End If
			Return result
		End Function
		...
		Private Function GetSavePath() As String
			...
			End If
			If Me._kyoten = HostSisanDataReceive.baseNum.Matuyama Then
				result = Path.Combine(Me._ftpSaveFolder, Me._ftpSisanFile)
			ElseIf Me._kyoten = HostSisanDataReceive.baseNum.Kumamoto Then
				result = Path.Combine(Me._ftpSaveFolder, Me._ftpSisanFile)
			End If
			Return result
		...
		Private Function CheckWritLineData(val As String) As Boolean
			...
			Return val.Length >= 272 OrElse flag
		End Function
		...
		Public _ftpSisanFile As String
		...
		Private _kyoten As HostSisanDataReceive.baseNum
