...
Namespace IbUkeharai.InquiryExec
	...
	Public Class HostBuhinSihdoDataReceive
		Public Sub New()
			Me._ftpBuhinSindoFile = Nothing
			Me._ftpBuhinSindoFile2 = New String() {Nothing, Nothing}
			Me._ftpSaveFolder = Nothing
			Me._ftpSaveFile = Nothing
			Me._enc = Encoding.GetEncoding("Shift_jis")
			Me._ftpkeppin = Nothing
			Me._testmode = False
			Me._kyoten = CType(0, HostBuhinSihdoDataReceive.baseNum)
		End Sub
		...
		Private Function CheckConfigParam() As Boolean
			...
			If Operators.CompareString(xmlConfData.Kyoten, String.Empty, False) = 0 Then
				Return False
			End If
			Me._kyoten = CType(Me.GetKyotenNo(xmlConfData.Kyoten), HostBuhinSihdoDataReceive.baseNum)
		...
		Private Function CheckSettingParam() As Boolean
			Return Not (Me._kyoten <> HostBuhinSihdoDataReceive.baseNum.Matuyama And Me._kyoten <> HostBuhinSihdoDataReceive.baseNum.Kumamoto) AndAlso Not Information.IsNothing(Me._ftpSaveFolder) AndAlso Not Information.IsNothing(Me._ftpSaveFile) AndAlso Me.SetFtpTestMode(Me._ftpkeppin.TestMode)
		End Function
		...
		Private Function KeppinDataReceive() As Boolean
			...
			If ftpCommon.IsDirectory(ftpdataUrl) Then
		...
		Private Function GetFTPDataUrl() As String
			...
			If Me._testmode Then
				Return result
			End If
			If Me._kyoten = HostBuhinSihdoDataReceive.baseNum.Matuyama Then
				result = Path.Combine(Me._ftpkeppin.FtpUrl, Me._ftpBuhinSindoFile)
			ElseIf Me._kyoten = HostBuhinSihdoDataReceive.baseNum.Kumamoto Then
				result = Path.Combine(Me._ftpkeppin.FtpUrl, Me._ftpBuhinSindoFile)
			End If
			Return result
		End Function
		...
		Private Function GetSavePath() As String
			...
			End If
			If Me._kyoten = HostBuhinSihdoDataReceive.baseNum.Matuyama Then
				result = Path.Combine(Me._ftpSaveFolder, Me._ftpBuhinSindoFile)
			ElseIf Me._kyoten = HostBuhinSihdoDataReceive.baseNum.Kumamoto Then
				result = Path.Combine(Me._ftpSaveFolder, Me._ftpBuhinSindoFile)
			End If
			Return result
		...
		Private Function CheckBackUpData(path As String) As Boolean
			...
			If Information.IsNothing(streamReader) Then
				Return result
			End If
		...
		Private Function CheckWritLineData(val As String) As Boolean
			...
			Return val.Length >= 209 OrElse flag
		End Function
		...
		Public _ftpBuhinSindoFile As String

		Public _ftpBuhinSindoFile2 As String()
		...
		Private _kyoten As HostBuhinSihdoDataReceive.baseNum
