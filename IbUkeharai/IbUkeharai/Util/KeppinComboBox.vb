...
Namespace IbUkeharai.Util
	...
	Public Class KeppinComboBox
		...
		<DebuggerNonUserCode()>
		Public Sub New()
			AddHandler MyBase.EnabledChanged, AddressOf Me.KeppinComboBox_EnabledChanged
			AddHandler MyBase.Validated, AddressOf Me.KeppinComboBox_Validated
		...
		Public Function SetItemListKeppin() As Boolean
			...
			Using sqlDataBase As New SqlDataBase(TopForm.ConfigData.xmlConfData.xDataBase)
				...
				If Not String.IsNullOrEmpty(sqldata) Then
					...
					Return False
				End If
				Me._itemList = New List(Of ItemInfomation2)()
				Try
					For Each dictionary As Dictionary(Of String, Object) In sqlDataBase.DbData.DataList
						If dictionary.ContainsKey(Me._columnCode) Then
							If dictionary.ContainsKey(Me._columnName) Then
								Me._itemList.Add(New ItemInfomation2(Conversions.ToString(dictionary(Me._columnCode)), Conversions.ToString(dictionary(Me._columnName))))
							End If
						End If
					Next
				Finally
		...
		Public Function SetToriItemList() As Boolean
			Dim itemListKeppin As String = "SELECT '' AS TORI_CD, '' AS TORI_NAME UNION ALL SELECT '全メーカー' AS TORI_CD, '全メーカー' AS TORI_NAME UNION ALL SELECT TORI_CD, TORI_NAME FROM [Ukeharai].[M_TORI] WHERE TORI_CD <> ''"
			Return Me.SetItemListKeppin(itemListKeppin)
		End Function

		Public Function SetItemListKeppin(sql As String) As Boolean
			If TopForm.ConfigData Is Nothing Then
				Return False
			End If
			If String.IsNullOrEmpty(Me._tableName) Or String.IsNullOrEmpty(Me._columnName) Then
			...
			Using sqlDataBase As New SqlDataBase(TopForm.ConfigData.xmlConfData.xDataBase)
				...
				Try
					For Each dictionary As Dictionary(Of String, Object) In sqlDataBase.DbData.DataList
						...
						Me._itemList.Add(New ItemInfomation2(code, name))
					Next
				Finally
					Dim enumerator As List(Of Dictionary(Of String, Object)).Enumerator = Nothing
					CType(enumerator, IDisposable).Dispose()
		...
		Private Sub KeppinComboBox_EnabledChanged(sender As Object, e As EventArgs)
		...
		Private Sub KeppinComboBox_Validated(sender As Object, e As EventArgs)
		...
		Private _itemList As List(Of ItemInfomation2)
